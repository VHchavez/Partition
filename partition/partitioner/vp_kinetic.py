...
def vp_kinetic(self):
    ...
    if self.optPart.kinetic_type == "vonweiz":
        ...
        # Define Initial guess for inverter
        # Let us not do initial guess for now. 

        # Sanity check
        # if self.inverter.dt == None:
        #     raise ValueError("Target Density Needed for inversion")
        ...
        #Load Data to inverter
        # nalpha = 0
        # nbeta  = 0
        # for ifrag in self.frags:
        #     nalpha += ifrag.nalpha
        #     nbeta += ifrag.nbeta
        # self.inverter.nalpha = nalpha
        # self.inverter.nbeta = nbeta
        ...
        if self.optPart.inv_method == 'oucarter':
            ...
            #Calculate guess in each method(?)
            ...
            self.inverter.invert(method=self.optPart.inv_method, opt_method=self.optPart.opt_method)
